...
const char* commandUrl = "https://water.monitoring.ehub.ph/api/api_get_command.php?station_id=41";
unsigned long lastUploadTime = 0;
unsigned long lastCommandCheckTime = 0;
const long COMMAND_CHECK_INTERVAL_MS = 10000; // Check for commands every 10 seconds
...
void loop() {
    ...
    // Check for server commands periodically (only when IDLE)
...
void checkServerCommand() {
    ...
    if (httpResponseCode == 200) {
        String response = http.getString();
        Serial.print("Server command response: ");
        Serial.println(response);

        // Parse JSON response
        // Simple parsing - look for "START_TEST" in response
        if (response.indexOf("START_TEST") > 0) {
            Serial.println("START_TEST command received from server!");
            if (currentState == IDLE) {
                currentState = WEB_START;
                Serial.println("Test cycle initiated by server command.");
            } else {
                Serial.println("Cannot start test - system busy.");
            }
        }
    } else if (httpResponseCode > 0) {
        Serial.printf("Command check failed. HTTP code: %d\n", httpResponseCode);
    } else {
        Serial.printf("Command check connection error: %s\n", http.errorToString(httpResponseCode).c_str());
    }
